def onTagChange(initialChange, newValue, previousValue, event, executionCount):
	"""Handle Ignition tag-change event for this resource."""
	if newValue.value == True:
		system.tag.writeAsync("[Otto_FleetManager]Triggers/Missions/Create/create_WF1_RV2", False)
		Otto_API.Missions.Post.createMission(
			templateTagPath="[Otto_FleetManager]Workflows/WF602_WetLabInnerdock/jsonString",
			robotTagPath="[Otto_FleetManager]Robots/AMPF_AMR_RV2/ID",
			missionName="Service Primus with RV2"
		)
